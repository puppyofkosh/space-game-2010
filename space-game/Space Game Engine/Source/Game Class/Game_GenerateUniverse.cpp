...
struct Segment;//forward declaration to prevent dependency erros
...
void Game::generateUniverse()
{
	...
	//for the colour generation
	float rColor = 0, gColor = 0, bColor = 0;
	int currentColor = 1;
	//Current color chooses which color element will be modified, 1 = red, 2 = g, 3 = b, etc

	for (int i = 0; i < govAmount; i++)
	{
		float colorInterval = (double)1/(segmentRows*segmentColumns);//We make as many colors as there are governments
		...
		currentColor++;//We need the color to change every time
		switch(currentColor%3 + 1)
		{
		case 1:
			rColor += (i+1) * colorInterval;
			break;
		case 2:
			gColor += (i+1) * colorInterval;
			break;
		case 3:
			bColor += (i+1) * colorInterval;
			break;
		default:
			currentColor++;
			break;
		};
		government.gov[i]->r = rColor;//The colors should be different everytime
		government.gov[i]->g = gColor;
		government.gov[i]->b = bColor;
	}
